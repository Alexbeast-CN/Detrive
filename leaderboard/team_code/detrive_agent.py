...
from detrive.model import DETRIVE
from detrive.config import GlobalConfig
from detrive.data import scale_and_crop_image
...
class DETRIVEAgent(autonomous_agent.AutonomousAgent):
	def setup(self, path_to_conf_file):
		...
		self.net = DETRIVE(self.config, 'cuda')
	...
	def run_step(self, input_data, timestamp):
		...
		img_array = scale_and_crop_image(Image.fromarray(tick_data['rgb']), scale=self.config.scale, crop=self.config.input_resolution).copy()

		if self.step < self.config.seq_len:
			rgb = torch.from_numpy(img_array).unsqueeze(0)
