...
class ImageCNN(nn.Module):
    ...
    def __init__(self, c_dim, normalize=True):
        ...
        self.features = DETR()
        self.features.load_state_dict(torch.load("/home/tim/AD_model/Detrive/model_ckpt/detr/detr_demo-da2a99e9.pth"))
        self.features.eval()
        
        self.linear_map = nn.Linear(64, 1) 
        self.linear = nn.Linear(756, c_dim)
    ...
    def forward(self, inputs):
        ...
        for x in inputs:
            ...
            z, map = self.features(x)
            map = self.linear_map(map)
            map = map.flatten(1)
            cat_features = torch.cat([z,map], dim=-1) # cat_features -> [b, 500+256]
            c += self.linear(cat_features)
        return c
